...
f=open("/mnt/minerva1/nlp/projects/sentiment10/Scripts/config.txt","r")
...
if __name__ == '__main__':
	...
	for x in os.listdir(config['dataSource']):
			y=x.split('.')[0].split('_')[0]

			date_time_obj = datetime.strptime(y, '%Y-%m-%d')
	...
	for x in sortedDictlist:
		print(x[-1][-2], end = ' - ')
		print(SentimentAnalyzer.Sentiment_from_url(x[0]), end=' - ')#ohodnoceni clanku podle url
		print(x[0])
	...
	for x in os.listdir(config['dataSource']):
	    ...
	    date_time_obj = datetime.strptime(y, '%Y-%m-%d')
	    if(date_time_obj>=datetime.strptime(date1, '%Y-%m-%d') and  date_time_obj<=datetime.strptime(date2,'%Y-%m-%d')): #kontrola jestli je clanek mezi zadanymi daty
	    	sys.stderr.write('\nZpracovávaný den:'+y+'\n')
	    	readReviews(config['dataSource']+x)


	#serazeni clanku recenzi podle pravdepodobnosti vyskytu recenze
	sortedDictlist=sorted(resultDict.items(), key=lambda j: j[1])
	print('\n')
	for x in sortedDictlist:
		print(x[-1][-2], end = ' - ')

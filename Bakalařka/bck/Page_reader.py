...
class Page_reader( object ):
    ...
    def __iter__( self ):
        ...
        for record in self._warc:
            ...
            if('Content-Encoding: gzip' in str(response)):
                if('HTTP/1.1 302' in str(response)):
                    responseList=str(response).split('HTTP/1.1 200 OK')
                    if('Content-Encoding: gzip' in responseList[1]):
                        continue
                else:
                    continue
                

            #stranky s timto content typem nejdou tokenizovat
            if ( 'Content-Type: image/jpeg' in str(response)):
                continue

            elif ( 'Content-Type: application/pdf' in str(response)):
                continue
